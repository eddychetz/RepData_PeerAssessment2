...
if (!"stormData.csv.bz2" %in% dir("./data/")) {
    print("hhhh")
    download.file("http://d396qusza40orc.cloudfront.net/repdata%2Fdata%2FStormData.csv.bz2", destfile = "data/FstormData.csv.bz2")
...
```{r}
if (!"FstormData" %in% ls()) {
    FstormData <- read.csv("data/FstormData.csv", sep = ",")
}
dim(FstormData)
head(FstormData, n = 2)
```
...
```{r}
if (dim(FstormData)[2] == 37) {
    FstormData$year <- as.numeric(format(as.Date(FstormData$BGN_DATE, format = "%m/%d/%Y %H:%M:%S"), "%Y"))
}
hist(FstormData$year, breaks = 30)
```  
...
```{r}
storm <- FstormData[FstormData$year >= 1995, ]
dim(storm)
```
...
```{r}
sortHelper <- function(fieldName, top = 15, dataset = FstormData) {
